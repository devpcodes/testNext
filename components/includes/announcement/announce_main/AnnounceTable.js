...
const AnnounceTable = ({ listData, getList, getData }) => {
    ...
    useEffect(() => {
        const GetNewData = async () => {
            try {
                ...
                getData(currentPage, pageSize, dataType, searchColumn, searchColumn2, searchWords).then(res => {
                ...
                render: (x, i) => {
                    ...
                    if (i.outLinkVal === '0') {
                        if (i.articleType === '0') {
                            return (
                                <a
                                    className="title_a aa"
                                    href={process.env.NEXT_PUBLIC_SUBPATH + '/AnnouncementPage?GUID=' + i.articleGUID}
                                >
                                    {tag}
                                    {x}
                                </a>
                            ...
                            return (
                                <a className="title_a bb" onClick={e => showOuterLinkPop(e, false, i.articleGUID)}>
                                    {tag}
                                    {x}
                                </a>
                            );
                        ...
                        return (
                            <a className="title_a cc" onClick={e => showOuterLinkPop(e, true, i.articleGUID)}>
                                {tag}
                                {x}
                            </a>
                        );
    ...
    const onFdSubmit2 = useCallback((confirm, val) => {
        //子類別過濾
        confirm();
        ...
        <>
            <div className="search_box">
                <Space direction="vertical">
                    <Search placeholder="輸入關鍵字" allowClear enterButton="搜尋" size="large" onSearch={onSearch} />
                </Space>
                <DropFilterCheckBoxM filterChangeM={filterChangeM} list_main={list_main} />
            </div>
            <TopTagBar current="" onClick={TopBarChange} mobleType={dimensions.width >= 768 ? false : true} />
            {isMobile ? (
                <AccountCard
                    dataSource={rows}
                    columns={columns}
            ...
            >
                <p>
                    ...
                    您若同意繼續進入該網站，請點選「確認」，不同意請點選「取消」，謝謝！
                </p>
            </Modal>
            <style jsx>{`
                .search_box {
            ...
            <style global jsx>{`
                ...
                .announce__container .ant-table-filter-trigger {
                    margin: 0;
                }
                .announce__container .ant-input {
                ...
                .announce__container .ant-table-container {
                    border: solid 1px #d7e0ef;
                    border-radius: 0;
                }
                .announce__container .ant-tooltip-inner {
                    color: white;
                ...
                .announce__container .ant-tooltip-arrow {
                    width: 25px;
                    height: 25px;
                    margin-top: -10px;
                }
                .announce__container .title_a {
                    display: block;
                    overflow: hidden;
                    white-space: nowrap;
                    text-overflow: ellipsis;
                }
                .announce__container .title_a:hover {
                    color: #daa360;
                }
                .announce__container .ant-table-tbody > tr > td:nth-child(4) {
                    max-width: 0px;
                }
                @media (max-width: 768px) {
                    .for_pc {
