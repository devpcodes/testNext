        ...
        init: function (options) {
            ...
            $(this).each(function () {
                ...
                if (!data) {
                    ...
                    var _totalpn =
                        data.options.total > 0 ? Math.floor((data.options.total - 1) / data.options.rpp) + 1 : 0;
                    ...
                    if (data.options.showinfobox && data.options.total > 0) {
                        ...
                        var _to = _fr + data.options.rpp - 1;
                        if (_to > data.options.total) _to = data.options.total;
                        var infomsg = LabCI.WP.CommonRC.doFmtMsg(rb.info, {
                            fr: _fr,
                            to: _to,
                    ...
                    if (_totalpn > 1) {
                        ...
                        for (
                            var i = startpageno;
                            i < startpageno + data.options.noofpagingbuttonsperblock && i <= _totalpn;
                            i++
                        ) {
                            var $gopage = $('<span/>')
                                .attr('pn', i)
                                .addClass('menulink pagebtn')
                                .html(i)
                                .appendTo($control);
                        ...
                        for (var i = 0; i < _totalpn; i++) {
                            ...
                            else goselect_html += '<option value="' + page + '">' + page + '</option>';
                        }
                        ...
                        $control.children('.prevbtn').click(function () {
                            var prevpn = data.options.pn - 1;
                            if (prevpn < 1) prevpn = 1;
                            if (data.options.goPage) data.options.goPage(prevpn);
                        });
                        $control.children('.nextbtn').click(function () {
                            var nextpn = data.options.pn + 1;
                            if (nextpn > _totalpn) nextpn = 1;
