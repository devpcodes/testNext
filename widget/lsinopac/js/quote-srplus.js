...
//        // Get ready
        ...
        _loadData: function () {
            ...
            this.$pageobj.loaddata("quotesrplusdata", "/data/srplus",
                    {
                        ric: ric,
                    ...
                    function (result) {
                        ...
                        if (result && result.data && result.data.responseCode !== "F") {
                            ...
                            if (result.data) {
                                ...
                                var msg = that.pageobj_rb.lbl.srplusavgscoreoutlook["as" + avgScore].msg.replace("@TICKER@", result.data.meta.ticker);
                                ...
                                that.$avgscore.find(".avgscore-outlook").html(that.pageobj_rb.lbl.srplusavgscoreoutlook["as" + avgScore].title);
                                that.$avgscore.find(".avgscore-outlookmsg").html(msg);



                                that.$avgscore.find(".avgscorebadge").append($("<img/>").attr("width", "100").attr("height", "135").attr("src", "../images/score/" + that.lang + "/s" + avgScore + ".png"));
                                ...
                                that.$borkerrating.find(".brokerratingscore").html(LabCI.WP.CommonRC.brokerrating2name(rate, that.lang)).removeClass("r1 r2 r3 r4 r5").addClass("r" + rateid);

                                var $rp = that.$borkerrating.find(".ratingpointer");
                                if (rateid != "NA") {
                                    $rp.attr("rate", rate).css("left", (rate - 1) * 23 + "%").show();
                                ...
                                if (that.$peer.is(":visible")) {
                                    ...
                                    that.$peer.find(".mktcapname").html(that.pageobj_rb.lbl.peerbox.srplusnamemap.mktcap[result.data.meta.MKTCAP_NAME]);
                                    that.$peer.find(".mktcapscore").html(result.data.meta.MKTCAP_TSS_AVG);
                                    that.$peer.find(".indexname").html(that.pageobj_rb.lbl.peerbox.srplusnamemap.index[result.data.meta.INDEX_NAME]);
                                    that.$peer.find(".indexscore").html(result.data.meta.INDEX_TSS_AVG);

                                    if (result.data.peer) {

                                        for (var key in result.data.peer) {
                                            var addedRow = $("<tr class='peer' style='cursor:pointer'; ric='" + result.data.peer[key].ric + "' symbol='"+result.data.peer[key].symbol+"' exchange='"+LabCI.WP.AppUtils.INTEGRATION_EXCHANGE_MAPPING[result.data.peer[key].exchsect]+"'><td>" + result.data.peer[key].nm + " (" + result.data.peer[key].symbol + ")" + "</td>" +
                                                    "<td style='text-align:center'>" + result.data.peer[key].AVG_SCORE_6M + "</td>" +
                                                    "<td style='text-align:center'>" + result.data.peer[key].AVG_SCORE_3M + "</td>" +
                                                    "<td style='text-align:center'>" + result.data.peer[key].AVG_SCORE_1M + "</td>" +
                                                    "<td style='text-align:center'>" + result.data.peer[key].AVG_SCORE_1W + "</td>" +
                                                    "<td style='text-align:center'>" + result.data.peer[key].AVG_SCORE + "</td>" +
                                                    "</tr>");
...
//
        ...
        _loadScoreTrendImage: function () {
            ...
            this.$pageobj.loaddata("srplus_scoretrendimage", "/data/srplus_scoretrendimage",
                    {
                        ric: ric,
...
//
        ...
        _loadCompScoreImage: function () {
            ...
            this.$pageobj.loaddata("srplus_compscoreimage", "/data/srplus_compscoreimage",
                    {
                        ric: ric,
                    ...
                    function (result) {
                        if (result && result.data && result.data.responseCode !== "F") {
                            if (result.data) {
                                that.$compscoreimage.append($("<img/>").attr("src", "data:image/png;base64," + result.data.loadCompScoreImage));
                            }
                            //     var now = new Date();
                            //     that.$avgscore.find("#x0").html(LabCI.WP.CommonRC.fmtDT(now, "FORMATPATTERN_DATE_SHORTYM", that.lang));
                            //     that.$avgscore.find("#x1").html(LabCI.WP.CommonRC.fmtDT(now.addYears(-1), "FORMATPATTERN_DATE_SHORTYM", that.lang));
