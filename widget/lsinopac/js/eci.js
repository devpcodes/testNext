...
////////////////////////////////////////////////////////////////////////////////
        ...
        this.$countryselect.$dropdown.append($button = $("<button/>").attr({value: "all", type:"button"}).addClass("dropdown-item").html(this.pageobj_rb.ecilist.all));
    ...
    initDateDropDown: function(){
        ...
        c_date = new Date();
        this.$dateselect.$dropdown.append($button = $("<button/>").attr({value: "all", type:"button"}).addClass("dropdown-item").html(this.pageobj_rb.ecilist.all));
        c_date.addMonths(-4);
        ...
        for (var i =0 ; i < 4 ; i++){
            ...
            m = date.getMonth() +1;
            if (m <9)
                val = "0" + m + "-" + date.getFullYear();
            else
                val = m + "-" + date.getFullYear();
            ...
            this.$dateselect.$dropdown.append($button = $("<button/>").attr({value: val, type:"button"}).addClass("dropdown-item").html(html));
        }
...
//        var that = this;
    ...
    _loaddata: function() {
        ...
        this._cleardata();
        var that = this;
        this.$pageobj.loaddata("eci", "/data/eci",
        {
            pid: "eci",
            lang: this.lang,
        ...
        function(result) {
            ...
            $.each(ecilist, function(i , r){
                ...
                if (!r.date && isNaN(tmp_date.getTime()))
                    tmp_date = new Date(r.date.replace(/-/g,"/"))
                
                flag = r.country.toLowerCase().replace(" ","_");
                $row.attr("data-ric",r.ric);
                $row.find(".data.date").html(tmp_date.getDate() + " " + LabCI.WP.CommonRC.getRes("month", that.lang)[tmp_date.getMonth()]);
                ...
                $row.find(".data.consensus_est").addClass(LabCI.Utils.getUpDownClass(r.reuters_survey)).html(r.reuters_survey);
                $row.on("click",function(){
                    
                    $("#chartCanvasMobile").remove();
                    $chart_m = $("<img/>").attr({id:"chartCanvasMobile", src: APP_CONFIG.DataAPIPath + "/lsinopac/ui/genchart.jsp?sizeid=C20&sym="+r.ric+"&style=1&int=6&per=15" }).addClass("m_o");
                    $(this).append($chart_m); 
                    $(this).show();
                    
                    $("#chartCanvas").remove();
                    $chart = $("<img/>").attr({id:"chartCanvas", src: APP_CONFIG.DataAPIPath + "/lsinopac/ui/genchart.jsp?sizeid=C20&sym="+r.ric+"&style=1&int=6&per=15" }).addClass("d_o");
