    ...
    LabCI.WP.QuoteChartPageObj = {
            ...
            if (this.$pageobj.find('.mobilechart').is(':visible')) {
                ...
                if (this.$pageobj.hasClass('changed')) {
                    if (this.chart.$this == null) {
                        ...
                        this.chart.mainchartobj = this.chart.chartframeobj.add(
                            chartFactory.PRICE_CHART,
                            this.chart.CHARTNAME_MAINCHART,
                            1,
                            1,
                            this.chart.$chartpane_canvasbox,
                            {
                                ...
                                crosshairStyle:
                                    ...
                                    chartFactory.CROSSHAIR_DASH,
                                crosshairFunc: this.crosshairFunc,
                                tooltipFunc: this.tooltipFunc,
                            },
                            1,
                        ); // use seq=1 to ensure that this is the 1st chart ;)
                        ...
                        this.chart.$periodoptselbox = this.chart.$periodoptbtn
                            .children('.chartoptselbox')
                            .detach()
                            .appendTo(this.$pageobj);
                        ...
                        this.chart.$typeoptselbox = this.chart.$typeoptbtn
                            .children('.chartoptselbox')
                            .detach()
                            .appendTo(this.$pageobj);
                        ...
                        this.chart.$taconfigbox = this.chart.$taoptbtn
                            .children('#taconfigbox')
                            .detach()
                            .appendTo(this.$pageobj);
        ...
        resizeMainChart: function () {
            ...
            if (this.chart.tachartobj) {
                this.chart.tachartobj.resize(this.chart.$chartpane_canvasbox.width(), this.chart._TAPANE_HEIGHT);
        ...
        hideCrosshair: function () {
            ...
            if (this.chart.$tooltipbox) this.chart.$tooltipbox.empty();
            if (this.chart.$tooltipbox2) this.chart.$tooltipbox2.empty();
        },
            ...
            if (res.date) {
                if (this.name === 'mainchart') {
                    str =
                        rb.dt +
                        ': <b>' +
                        LabCI.WP.CommonRC.fmtDT(res.date, that.chart._curdataopts._dtfmt, that.lang) +
                        '</b> ';
                    if (that._iswidescrn) {
                        str +=
                            rb.o +
                        ...
                        if (taParam != null && taParam[0] == 'BB') {
                            ...
                            str +=
                                ...
                                ',' +
                                taParam[2] +
                                "σ: <span style='color:#555555'><b>MA " +
                                bb.m +
                                '</b> (' +
                                taParam[1] +
                                ')</span> <b>' +
                                taParam[2] +
                                "σ</b> <span style='color:#0091ea'><b>" +
                                bb.u +
                                "</b> (UB)</span> <span style='color:#d0021b'><b>" +
                                bb.l +
                                '</b> (LB)</span>';
                            ...
                            str +=
                                ...
                                ',' +
                                taParam[3] +
                                ": <span style='color:#9933FF'><b>" +
                                sma1 +
                                '</b> (' +
                                taParam[1] +
                                ")</span> <span style='color:#FFB0B0'><b>" +
                                sma2 +
                                '</b> (' +
                                taParam[2] +
                                ")</span> <span style='color:#449999'><b>" +
                                sma3 +
                                '</b> (' +
                                taParam[3] +
                                ')</span>';
                        ...
                        if (that._iswidescrn) {
                            ...
                            if (this.name === 'RSI' && taParam != null) {
                                str =
                                    ...
                                    '</b> EMA-' +
                                    taParam[1] +
                                    ": <span style='color:#00a546'><b>" +
                                    res['RSI_2_' + taParam[1]] +
                                    '</b> (EMA-' +
                                    taParam[1] +
                                    ')</span>'; //var(--stanchart-green)
                                ...
                                str =
                                    ...
                                    ',' +
                                    taParam[1] +
                                    ": <span style='color:#d0021b'><b>" +
                                    res.macd +
                                    "</b> (MACD)</span> <span style='color:#555555'><b>" +
                                    res.signal +
                                    '</b> (' +
                                    rb.signal +
                                    ')</span>';
                            ...
                            //else if (res.pctK) {
                                ...
                                str =
                                    ...
                                    ',' +
                                    taParam[2] +
                                    ": <span style='color:#d0021b'><b>%K " +
                                    res.pctK +
                                    "</b></span> <span style='color:#0080FF'><b>%D " +
                                    res.pctD +
                                    '</b></span>';
        ...
        _chart_changeperiod: function (period) {
            ...
            this.chart.$periodoptselbox
                .children(".chartoptbtn[tabid='" + this.chart.curperiod + "']")
                .addClass('selected');
            //    this.resizeImpl();
        },
        ...
        _chart_changetype: function (type) {
            ...
            // Save it and apply
            if (type) this.chart.curtype = type;
            switch (this.chart.curtype) {
                case 'bar':
                    var styleobj = {
        ...
        _OVERLAY_TALIST: ['nooverlay', 'SMA', 'BB', 'SAR'],
        _TAPANE_TALIST: ['notapane', 'RSI', 'MACD', 'SKD', 'FKD'],
        _chart_changeta: function (ta) {
            ...
            if ($.inArray(ta, this._OVERLAY_TALIST) !== -1) {
                _isoverlay = true;
                if (this.chart.curta.overlay) this._chart_removeTAOverlay(this.chart.curta.overlay);
                this.chart.curta.overlay = ta; // save the new one
            } else {
                _isoverlay = false;
                if (this.chart.curta.tapane) this._chart_removeTAPane(this.chart.curta.tapane);
                this.chart.curta.tapane = ta; // save the new one
            }

            // Apply the new TA settings
            var $panel = _isoverlay
                ? this.chart.$taconfigbox.children('#overlay')
                : this.chart.$taconfigbox.children('#tapane');
        ...
        _chart_addTAPane: function (ta) {
            // State check
            if (!this.chart.mainchartobj) return;
            ...
            switch (ta) {
                case 'RSI':
                    ...
                    if (p.length > 0) {
                        ...
                        if (p !== null) {
                            this.chart.tachartobj = this.chart.chartframeobj.add(
                                chartFactory.PRICE_CHART,
                                chartFactory.RSI,
                                this.chart.$this.width(),
                                this.chart._TAPANE_HEIGHT,
                                this.chart.$chartpane_canvasbox,
                                this._getMainChartOptions.call(this),
                                100,
                            );
                            this.chart.tachartobj.setGridStyle(chartFactory.GRID_LABEL_X);
                            this.chart.tachartobj.data(this.chart.mainchartobj.series);

                            this.chart.tachartobj.addPriceTracker2('lowerRSI', 25, {
                                color: '#000080',
                                textColor: '#333333',
                                dashstyle: chartFactory.TRACKER_DASH,
                            });
                            this.chart.tachartobj.addPriceTracker2('upperRSI', 75, {
                                color: '#000080',
                                textColor: '#333333',
                                dashstyle: chartFactory.TRACKER_DASH,
                            });
                            this.chart.tachartobj.addPriceTracker2('y50', 50, {
                                color: '#EEEEEE',
                                textColor: '#333333',
                                dash: 0,
                            });
                ...
                case 'SKD':
                    ...
                    if (p.length > 0) {
                        ...
                        if (p !== null && p1 !== null) {
                            ...
                            this.chart.tachartobj.addPriceTracker2('lowerSKD', 25, {
                                color: '#000080',
                                textColor: '#333333',
                                dashstyle: chartFactory.TRACKER_DASH,
                            });
                            this.chart.tachartobj.addPriceTracker2('upperSKD', 75, {
                                color: '#000080',
                                textColor: '#333333',
                                dashstyle: chartFactory.TRACKER_DASH,
                            });
                ...
                case 'FKD':
                    ...
                    if (p.length > 0) {
                        ...
                        if (p !== null && p1 !== null) {
                            ...
                            this.chart.tachartobj.addPriceTracker2('lowerFKD', 25, {
                                color: '#000080',
                                textColor: '#333333',
                                dashstyle: chartFactory.TRACKER_DASH,
                            });
                            this.chart.tachartobj.addPriceTracker2('upperFKD', 75, {
                                color: '#000080',
                                textColor: '#333333',
                                dashstyle: chartFactory.TRACKER_DASH,
                            });
            ...
            if (this.chart.tachartobj) this.chart.tachartobj.c5pobj = this; // ensure the reference to this c5pobj is saved, if there is a TAChartObj (i.e. not removed ;)
            ...
            switch (ta) {
                case 'SMA':
                    ...
                    for (var i = 0; i < drawers.length; i++) {
                        if (drawers[i].obj.name.startsWith(chartFactory.SMA)) ids.push(drawers[i].obj.name);
                    }
                ...
                case 'BB':
                    ...
                    var drawers = this.chart.mainchartobj.drawers;
                    for (var i = 0; i < drawers.length; i++) {
                        if (drawers[i].obj.name.startsWith(chartFactory.BOLLINGERBANDS)) ids.push(drawers[i].obj.name);
                    }
                    for (var i = 0; i < ids.length; i++) {
                        this.chart.mainchartobj.removeDrawer(ids[i]);
                    }
        ...
        _chart_loaddata: function () {
            ...
            this.chart.$this.loaddata(
                'loadchartdata',
                '/data/' + _dataopts._c5api,
                {
        ...
        _setUILabels: function () {
            ...
            $.each(this.$pageobj.find('[lbl]'), function () {
                ...
                var id = $obj.attr('lbl');
                var lbl = lblrb[id];
                if (!lbl) lbl = LabCI.WP.CommonRC.getMsg(id, LabCI.getLang());
                $obj.html(lbl);
            });
