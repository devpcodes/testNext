            ...
            if (comp == '' || comp == undefined || comp == null) {
                ...
                if (opt.period.indexOf('CUSTOM') == 0) {
                    url +=
                        '&period=CUSTOM' +
                        '&from=' +
                        opt.period.split(',')[1].split('-')[0] +
                        '&to=' +
                        opt.period.split(',')[1].split('-')[1];
                } else {
            ...
            if (comp == '' || comp == undefined || comp == null) {
                ...
                if (options.period.indexOf('CUSTOM') == 0) {
                    url +=
                        '&period=CUSTOM' +
                        '&from=' +
                        options.period.split(',')[1].split('-')[0] +
                        '&to=' +
                        options.period.split(',')[1].split('-')[1];
                } else {
        ...
        _compareChartCallbackHandler: function (json) {
            ...
            if (json && !jQuery.isEmptyObject(json)) {
                ...
                function compareChartMoveHandler(x, y, ele, isRPChecked) {
                    ...
                    var compareChartTooltip = $("<div id='compareChartTooltip'/>")
                        .css({
                            position: 'absolute',
                            top: y + 'px',
                            //left: (+nearX) + "px",
