...
(function ($) {
    ...
    LabCI.Utils.CONSOLELOGGER = {
        ...
        _logmsg2consoleloggerdialog: function (msg) {
            ...
            var isbottom = true;
            if (
                this._$consoleloggerdialog[0].scrollTop + this._$consoleloggerdialog[0].offsetHeight <
                this._$consoleloggerdialog[0].scrollHeight
            ) {
                isbottom = false;
            }
                    ...
                    if (jqXHR.status === 401) {
                        ...
                        if (data && _thisqid == data.qid) {
                            ...
                            if (textStatus == 'timeout') {
                                // Timeout?!
                                result = {
                                    data: { responseCode: 'F', reasonCode: LabCI.SSDL._API_RC.REQUEST_TIMEOUT },
                                };
                                onReceiveInvalidData(result.data);
...
LabCI.WP.CommonRC = {
    getRes: function (key, lang) {
        ...
        if (
            !lang ||
            typeof LabCI.WP.CommonRC[lang] === 'undefined' ||
            typeof LabCI.WP.CommonRC[lang][key] === 'undefined'
        )
            return key;
