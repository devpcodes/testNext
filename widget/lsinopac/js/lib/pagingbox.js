...
//                        }
                        ...
                        var $goprev = $("<div/>").attr("toolbuttongroup", toolbuttongroupname_pageto).attr("id","prev").addClass("pagetobutton pushbutton").html($("<span/>").html(labels.prev)).appendTo($pagingcontrol);
...
//                        if (block == 0 || noofblocks == 1) $goprev10.css("display", "none"); //$goprev10.css("visibility", "hidden");
                        ...
                        for (var i = startpageno; i < startpageno + data.options.noofpagingbuttonsperblock; i++) {
                            var $gopage = $("<div/>").attr("toolbuttongroup", toolbuttongroupname_paging).attr("id", i).addClass("pagingbutton pushbutton").html($("<span/>").html(i)).appendTo($pagingcontrol);
...
//                        if (block == noofblocks-1 || noofblocks == 1) $gonext10.css("display", "none"); //$gonext10.css("visibility", "hidden");
                        ...
                        var $gonext = $("<div/>").attr("toolbuttongroup", toolbuttongroupname_pageto).attr("id","next").addClass("pagetobutton pushbutton").html($("<span/>").html(labels.next)).appendTo($pagingcontrol);
                            ...
                            if (data.options.goPage) {
                                ...
                                switch (id) {
                                    case "first":
                                        ...
                                        break;
                                    case "prev":
                                        pageno = data.options.currentpage - 1; if (pageno < 1) pageno = 1;
                                        break;
                                    case "prev10":
                                        pageno = data.options.currentpage - data.options.noofpagingbuttonsperblock; if (pageno < 1) pageno = 1;
                                        break;
                                    case "next10":
                                        pageno = data.options.currentpage + data.options.noofpagingbuttonsperblock; if (pageno > data.options.totalpages) pageno = data.options.totalpages;
                                        break;
                                    case "next":
                                        pageno = data.options.currentpage + 1; if (pageno > data.options.totalpages) pageno = data.options.totalpages;
                                        break;
                                    case "last":
