    ...
    var methods = {
        ...
        init: function(options) {
            ...
            $(this).each(function() {
                ...
                if (!data) {
                    ...
                    var $celltmpl = $("<div/>").addClass("heatmapcell hmcell").append($("<div/>").addClass("heatmapcelltextbox"));
        ...
        _createNewCell: function(data, celldata) {
            ...
            if (celldata) {
                ...
                $hmc.attr({ id: celldata.id, ric: celldata.ric, trbc: celldata.trbc }).css({ left: celldata.left, width: celldata.width, top: celldata.top, height: celldata.height }).addClass(LabCI.Utils.getHeatmapTemp(celldata.value));
                ...
                if (data.options.onClick) {
                    $hmc.click(function(e) {
                        if ($("#itemlist").hasClass("sct")){
                            ...
                            data.options.onClick({ symbol: celldata.id, exchange: LabCI.WP.AppUtils.INTEGRATION_EXCHANGE_MAPPING[celldata.exchsect]}, e);
                        }
                    });
                }
            }
...
//                        $hmctbx.css({ left: ($hmc.width() - $hmctbx.width()) / 2, top: ($hmc.height() - $hmctbx.height()) / 2 });
                    ...
                    if (idorderlist) {
                        $.each(idorderlist, function(index, id) {
                            var $hmc = data.$contentpane.children("#"+id).addClass("order"+index).detach();
                            data.$contentpane.append($hmc);
