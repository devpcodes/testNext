    ...
    LabCI.WP.SctHmPageObj = {
        ...
        initImpl: function () {
            ...
            this.$heatmapcanvas = this.$heatmapbox.children('#heatmapcanvas').heatmapbox({
                ...
                renderCell: function ($hmc, $hmctbx, celldata) {
                    ...
                    $hmctbx.append(
                        $('<div/>').addClass('cellname').html($("<a href='javascript:void(0)'/>").html(celldata.name)),
                    );
        ...
        switchmktper: function (mkt, per, dorefresh) {
            ...
            this.$mktbarcells
                ...
                .filter("[id='" + this.curmkt + "']")
                .addClass('selected');
            this.$perbarcells
                .removeClass('selected')
                .filter("[id='" + this.curper + "']")
                .addClass('selected');
        ...
        _populateheatmapdata: function () {
            ...
            if (_heatmap2Ddata.length > 0) {
                ...
                that.$heatmapcanvas.heatmapbox(
                    'addCells',
                    _heatmap2Ddata,
                    that.cursizetype === 'am' ? that.curdata.orderlist_am : that.curdata.orderlist_mc,
                );
                ...
                if (top3.length > 0) {
                    $.each(top3, function (i, item) {
                        ...
                        $item
                            .children('.pc')
                            .setValue(item.value, '%')
                            .removeClass('upval downval')
                            .addClass(LabCI.Utils.getUpDownClass(item.value));
                    });
                } else {
                ...
                $.each(_allheatmapdata, function (i, item) {
                    ...
                    if ($('#itemlist').hasClass('stk')) {
                        ...
                        $row.attr({
                            symbol: item.id,
                            exchange: LabCI.WP.AppUtils.INTEGRATION_EXCHANGE_MAPPING[item.exchsect],
                        });
                    }
                    ...
                    if (that.cursct) {
                        ...
                        if (item.exchsect === 'SHZ' && item.id.startsWith('3'))
                            $row.children('.textlink').removeClass('textlink');
                    }
