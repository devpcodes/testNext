...
// The main object definition is here...
    LabCI.WP.PSPageObj = {
        ...
        initImpl: function () {
            ...
            this.$input = this.$pageobj.children("input")
                    .focus(function () {
                        ...
                        if (keyCode === 38 || keyCode === 40) { // up (38) | down (40)
                            ...
                            if (that.$resultdropdown.is(":visible")) {
                                ...
                                if ($entry.length > 0) {
                                    ...
                                    var $cur = $entry.filter(".hover");
                                    if ($cur.length === 0)
                                        $cur = $entry.first(); // no one was selected, pick the first one la
                                    $cur.click();
                                }
                            }
//                    else {
...
//                    }
                            ...
                            if (keyCode !== 37 && keyCode !== 39) { // left (37) | right (39) | ..., since the input was not changed, no need clear/hide ;)
                                that.$resultdropdown.hide();
                            }

                            // Fire search at a delayed time...
                            that.$input.delaycall("powersearch", function () {
                                ...
                                if (!that.$input.hasClass("placeholder") && _key.length >= that._PS_MINCHARS) {
                                    ...
                                    that.$pageobj.addClass("loading").loaddata("ps", "/data/ps",
                                            {
...
//                                    if (LabCI.Utils.CONSOLELOGGER.isdebug) LabCI.Utils.CONSOLELOGGER.debug("that.$input powersearch: " + JSON.stringify(result));
                                                if (result && result.data && result.data.responseCode === "S") {
                                                    ...
                                                    if (data.length > 0) {
                                                       ...
                                                       //     console.log(entry);
                                                            var $entry = $("<div class='result'><div class='symbol'>" + entry.symbol + "</div><div class='nm'>" + entry.nm + "</div><div class='exch'>" + that.pageobj_rb[entry.exchsect] + "</div></div>").data(entry)
                                                                    .click(function () {
                                                                        // Choose this...
                                                                        that.$resultdropdown.hide();
                                                        ...
                                                        // Show error message
                                                        that.$resultdropdown.html("<div class='result empty'>" + that.pageobj_rb.empty + "</span>");
                                                    }
                                                } else {
                                                    // Show error message
                                                    that.$resultdropdown.html("<div class='result empty'>" + that.pageobj_rb.empty + "</span>");
