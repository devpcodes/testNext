(function ($) {
    ...
    LabCI.WP.WorldNavPageObj = {
        ...
        _LABELS: {
            ...
            'msg.maxpopoutscreen':
                'Max number popout windows have been opened. Please close some before opening another.',
        ...
        initImpl: function () {
            ...
            for (var key in that._MAPQUOTEBOX_MAP) {
                var tmpObj = $(
                    "<img style='width: 630px' src='../images/worldnav/world_map3.png'/><div class='mapquoteboxlayer'></div>",
                );
                this.$mapregionbox.append(tmpObj);

                for (var key2 in that._MAPQUOTEBOX_MAP[key]) {
                    var tmpObj2 = $(
                        '<div class="mapquotebox box_' +
                            key2 +
                            '" boxId ="' +
                            key2 +
                            '"> \
                            <div class="title"><div class="ccname font1 ' +
                            that.IDXNAME[key2] +
                            'slabel">' +
                            that.IDXNAME[key2] +
                            '</div><div class="stream time td" format="date">--/--</div><div class="stream time tm" format="timenoreplace">--:--</div></div> \
                            <div class="content"> \
                ...
                that.$mpanel.append(
                    '<div class="mquoteinfo mbox_' +
                        ...
                        '"><div class="row namerow"> \
                            <div class="name col-8 text-left ' +
                        index +
                        '" style="padding-right: 0px">' +
                        that.pageobj_rb.lbl[that.IDXNAME[index] + 'label'] +
                        '</div> \
        ...
        resizeImpl: function () {
            ...
            if (
                (this.$pageobj.find('.desktoptemplate').is(':visible') && this.mode != 'desktop') ||
                                ...
                                if (that.mode == 'desktop') {
                                    ...
                                    if (fidmap) {
                                        ...
                                        $databox
                                            .find('.td')
                                            .attr('fid', fidmap.td)
                                            .attr('timeref', fidmap.tm)
                                            .html(formatShortDate(ricdata.td));
                                        $databox
                                            .find('.tm')
                                            .attr('fid', fidmap.tm)
                                            .html(formatShortTime(ricdata.tm) + ', ');
                                        $databox
                                            .find('.ls')
                                            .attr('fid', fidmap.ls)
                                            .attr('dp', ricdata.dp)
                                            .attr('flashfid', fidmap.nc + ' ' + fidmap.pc)
                                            .html(ricdata.ls);
                                        $databox
                                            .find('.nc')
                                            .attr('fid', fidmap.nc)
                                            ...
                                            .setValue(ricdata.nc, null, true);
                                        $databox
                                            .find('.pc')
                                            .attr('fid', fidmap.pc)
                                            .addClass(updownclass)
                                            .setValue(ricdata.pc, '%', true);
                                    }
                                    ...
                                    if (fidmap) {
                                        ...
                                        $databox
                                            ...
                                            .attr('timeref', fidmap.tm)
                                            .html(
                                                formatShortTime(ricdata.tm) + ',&nbsp;' + formatShortDate(ricdata.td),
                                            );
                        ...
                        if (result.data.datalist && result.data.datalist.length > 0) {
                            $.each(result.data.datalist, function (index, ricdata) {
                                ...
                                if (fidmap) {
                                    ...
                                    that.$mapQuoteBox
                                        .find('.name')
                                        .html(that.pageobj_rb.lbl[that.IDXNAME[that.RIC2BOXID[ric]] + 'label']);
                                    that.$mapQuoteBox
                                        .find('.date')
                                        .html(
                                            formatShortTime(ricdata.tm) +
                                                ',&nbsp;&nbsp; ' +
                                                formatShortDate(ricdata.td),
                                        );
                                    ...
                                    if (ric == '.HSI') {
                                        that.$quotedetail
                                            ...
                                            .html(
                                                ricdata.mktam +
                                                    LabCI.WP.CommonRC.unitscale2name(ricdata.mktamunit, that.lang) +
                                                    that.pageobj_rb.lbl['unit'],
                                            );
                                    } else {
                                        that.$quotedetail
                                            .find('.turnover')
                                    ...
                                    that.$quotedetail
                                        ...
                                        .removeClass('upval')
                                        .removeClass('downval')
                                        .addClass(getUpDownClass(ricdata.p1m))
                                        .setValue(ricdata.p1m, '%', true);
                                    that.$quotedetail
                                        .find('.r3M')
                                        .removeClass('upval')
                                        .removeClass('downval')
                                        .addClass(getUpDownClass(ricdata.p3m))
                                        .setValue(ricdata.p3m, '%', true);
                                    that.$quotedetail
                                        .find('.r6M')
                                        .removeClass('upval')
                                        .removeClass('downval')
                                        .addClass(getUpDownClass(ricdata.p6m))
                                        .setValue(ricdata.p6m, '%', true);
                                    that.$quotedetail
                                        .find('.r1Y')
                                        .removeClass('upval')
                                        .removeClass('downval')
                                        .addClass(getUpDownClass(ricdata.p1y))
                                        .setValue(ricdata.p1y, '%', true);
                                }
                            });
                        }
                    } else {
