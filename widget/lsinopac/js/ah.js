...
(function ($) {
    ...
    LabCI.WP.AHPageObj = {
        ...
        initImpl: function () {
            ...
            this.$ahrsortbtn = this.$stocklist.$this
                .children('.headerrow')
                .children('.ahr')
                .click(function () {
            ...
            if (total_d > 0) {
                ...
                if (total_d > this.DEFAULT_RECPERPAGE) {
                    ...
                    var fnGoPage_d = function (pn) {
                        ...
                        if (endidx > total_d) endidx = total_d;
                        for (var i = startidx; i < endidx; i++) {
                            $rows_d.eq(i).show();
                        }

                        // Setup a new paging bar of the corresponding page
                        that.$stocklist.$pagingbar_d
                            .show()
                            .pagingbar('destroy')
                            .pagingbar({
                                pn: that.curpn,
            ...
            if (total_p > 0) {
                ...
                if (total_p > this.DEFAULT_RECPERPAGE) {
                    ...
                    var fnGoPage_p = function (pn) {
                        ...
                        var startidx = (that.curpn - 1) * that.DEFAULT_RECPERPAGE;
                        var endidx = startidx + that.DEFAULT_RECPERPAGE;
                        if (endidx > total_p) endidx = total_p;
                        for (var i = startidx; i < endidx; i++) {
                            $rows_p.eq(i).show();
                        }

                        // Setup a new paging bar of the corresponding page
                        that.$stocklist.$pagingbar_p
                            .show()
                            .pagingbar('destroy')
                            .pagingbar({
                                pn: that.curpn,
                    ...
                    if (result && result.data && result.data.responseCode !== 'F') {
                        ...
                        if (d.h) {
                            if (d.h.length > 8) {
                                ...
                                that.$stocklist.$lastupdate.Hshr.text(
                                    LabCI.WP.CommonRC.fmtDT(dt, 'FORMATPATTERN_DATETIME_SHORTMD_DCLM', that.lang),
                                );
                            } else {
                                dt = Date.parseExact(d.h, 'yyyyMMdd');
                                that.$stocklist.$lastupdate.Hshr.text(
                                    LabCI.WP.CommonRC.fmtDT(dt, 'FORMATPATTERN_DATE_SHORTYMD', that.lang),
                                );
                            }
                        }
                        if (d.a) {
                            if (d.a.length > 8) {
                                ...
                                that.$stocklist.$lastupdate.Ashr.text(
                                    LabCI.WP.CommonRC.fmtDT(dt, 'FORMATPATTERN_DATETIME_SHORTMD_DCLM', that.lang),
                                );
                            } else {
                        ...
                        that.$stocklist.$fx.lastupdate.text(
                            LabCI.WP.CommonRC.fmtDT(dt, 'FORMATPATTERN_DATE_SHORTYMD', that.lang),
                        );
                        ...
                        $.each(d, function (i, r) {
                            ...
                            $o.find('.ls_CNY').text(
                                parseFloat(parseFloat(s.ls) / parseFloat(that.curdata.fx.hc)).toFixed(2),
                            );
                            ...
                            if (20.0 < ahr) {
                                p3++;
                                $row.addClass('p3');
                            } else if (10.0 < ahr && ahr <= 20.0) {
                                p2++;
                                $row.addClass('p2');
                            } else if (0.0 <= ahr && ahr <= 10.0) {
                                p1++;
                                $row.addClass('p1');
                            } else if (-10.0 <= ahr && ahr < 0.0) {
                                d1++;
                                $row.addClass('d1');
                            } else if (-20.0 <= ahr && ahr < -10.0) {
                                d2++;
                                $row.addClass('d2');
                            } else if (ahr < -20.0) {
                                d3++;
                        ...
                        that.$pageobj
                            .find('#d_p_sacle_wrapper .d_p_sacle')
                            .attr('data-sacle', Math.round(((d_count / (d_count + p_count)) * 100) / 5));
        ...
        _setUILabels: function () {
            ...
            this.$ahrsortbtn
                .html(rb.ahr)
                .find('.sort')
                .html(this.curord === 'asc' ? this._UPARR : this._DNARR);
